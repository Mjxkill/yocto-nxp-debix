...
\title{\textbf{Plateforme Mastering Live Debix}\\[6pt]
  \Large Spécification Phase 1a.2 --- \textbf{V1}\\[4pt]
  \large Tap HP\_Monitor post-effets OUT\\
  \normalsize Pipeline capture piggyback sur \texttt{PIPELINE\_PLAYBACK\_SCHED\_COMP\_6}}
\author{Document de référence technique}
\date{2026-04-23 --- rev V1}
...
# DAPM graph playback : host -> effets -> demux -> {DAI, tap}
...
# Exports : SOURCE vers DAI (B4), DEMUX exposé vers cross-pipeline (B5)
indir(`define', concat(`PIPELINE_SOURCE_', PIPELINE_ID), N_BUFFER(4))
indir(`define', concat(`PIPELINE_DEMUX_',  PIPELINE_ID), N_BUFFER(5))
...
# PAS DE W_PIPELINE : piggyback utilisera sched_comp de PIPE 6
# (pas de pipe-dai-*.m4 car pas de DAI réel, tap interne)

# Graph : PCM capture <- buffer (le buffer sera source du demux cross-pipeline)
...
# 12e arg SCHEDULE_TIME_DOMAIN_DMA (hérite implicitement via sched_comp)
# 13e arg PIPELINE_PLAYBACK_SCHED_COMP_6 = N_DAI_OUT pour PIPE 6
...
# DAI_ADD : SAI7 RX (PIPE 1) + SAI7 TX (PIPE 6)

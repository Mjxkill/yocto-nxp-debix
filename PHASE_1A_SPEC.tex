...
\title{\textbf{Plateforme Mastering Live Debix}\\[6pt]
  \Large Architecture C --- Spécification complète Phase 1a\\[4pt]
...
\subsection{Mapping pipeline $\leftrightarrow$ PCM}
...
\midrule
PIPE 1 + DAI\_ADD capture & \texttt{SAI\_Capture} & 0 (via DUPLEX) & Dry mics 8ch \\
PIPE 6 + DAI\_ADD playback & \texttt{SAI\_Playback} & 0 (via DUPLEX) & Direct 8ch vers DAC \\
PIPE 3 (master chain in) & \texttt{Source\_Play} & 1 & Entrée stéréo master \\
PIPE 5 (master chain out) & \texttt{Master\_Tap} & 2 & Sortie post-master (NPU) \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{Squelette m4 complet (Phase 1a.1 MVP)}

Fichier : \texttt{sof/tools/topology/topology1/sof-imx8mp-tac5212-masterlite.m4}

\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
#
# Topology Phase 1a MVP - 4 PCMs : SAI_Capture, SAI_Playback,
...
# ---------------------------------------------------------------------
# PIPE 1 : Dry mics capture (8ch) - baseline drc2ms equivalent
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# PIPE 6 : Direct playback (8ch) - baseline drc2ms equivalent
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# PIPE 3 : Master chain input side (HOST -> mbdrc -> volume -> drc -> buf)
# Utilise un pipeline CUSTOM local qui instancie la chaîne stéréo
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# PIPE 5 : Master chain output side (buf -> HOST Master_Tap)
# Lit depuis le buffer de sortie de PIPE 3 via DAPM link
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# DAI_ADD : SAI7 RX + TX (identiques a drc2ms.m4 qui fonctionne)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# PCM exports (les noms declares ici apparaissent dans /proc/asound/)
# ---------------------------------------------------------------------
# PCM 0 duplex : SAI_Capture (cap) + SAI_Playback (play)
PCM_DUPLEX_ADD(TAC5212_SAI, 0, PIPELINE_PCM_6, PIPELINE_PCM_1)

# PCM 1 playback only : Source_Play (entree master chain)
PCM_PLAYBACK_ADD(Source_Play, 1, PIPELINE_PCM_3)

# PCM 2 capture only : Master_Tap (sortie post master chain)
PCM_CAPTURE_ADD(Master_Tap, 2, PIPELINE_PCM_5)

# ---------------------------------------------------------------------
# Graph : connecter le buffer sink de PIPE 3 au source de PIPE 5
# (master chain output -> master tap input)
# ---------------------------------------------------------------------
...
\paragraph{\texttt{sof-imx8mp-tac5212-pipe-masterchain.m4}}
Pipeline d'entrée de la chaîne master. Squelette :
\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
# Master chain input: host PCM -> mbdrc -> volume -> drc -> buf (shared)
include(`utils.m4')
include(`buffer.m4')
include(`pcm.m4')
include(`pipeline.m4')
include(`bytecontrol.m4')
include(`multiband_drc.m4')  # multiband_drc widget macro
include(`volume.m4')          # volume widget macro
include(`drc.m4')             # drc widget macro
...
C_CONTROLBYTES(MBDRC_CTRL, PIPELINE_ID, ..., MBDRC_priv)
W_MULTIBAND_DRC(0, PIPELINE_FORMAT, 2, 2, SCHEDULE_CORE,
    LIST(`   ', "MBDRC_CTRL"))

W_VOLUME(0, PIPELINE_FORMAT, 2, 2, SCHEDULE_CORE, 0, 0)
...
C_CONTROLBYTES(DRC_CTRL, PIPELINE_ID, ..., DRC_priv)
W_DRC(0, PIPELINE_FORMAT, 2, 2, SCHEDULE_CORE,
    LIST(`   ', "DRC_CTRL"))

# Buffers
W_BUFFER(0, COMP_BUFFER_SIZE(2, ...))  # host buffer
W_BUFFER(1, COMP_BUFFER_SIZE(2, ...))  # between mbdrc and volume
W_BUFFER(2, COMP_BUFFER_SIZE(2, ...))  # between volume and drc
W_BUFFER(3, COMP_BUFFER_SIZE(2, ...))  # OUTPUT (shared with PIPE 5)
...
# Export: buffer 3 est le SOURCE de la pipeline (visible au graph externe)
...
\paragraph{\texttt{sof-imx8mp-tac5212-pipe-mastertap.m4}}
Pipeline de sortie côté Master\_Tap, simple capture depuis un buffer externe :
\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
# Master tap: buf (shared) -> host PCM Master_Tap
include(`utils.m4')
include(`buffer.m4')
include(`pcm.m4')
include(`pipeline.m4')

# Host PCM capture endpoint
W_PCM_CAPTURE(PCM_ID, Master Tap, 0, 2, SCHEDULE_CORE)

# Buffer (sera lie au N_BUFFER(3) de la masterchain via DAPM externe)
...
# Export: buffer 0 est le SINK de la pipeline (attendu par le graph externe)
...
\subsection{T4 --- Null test bypass}
...
# Jouer et capturer
arecord -D plughw:softac5212tdm,Master_Tap -c 2 -r 48000 -f S32_LE \
        -d 5 /root/tests/out/bypass.wav &
sleep 1
aplay -D plughw:softac5212tdm,Source_Play /root/tests/in/tone440.wav
wait

# Null test : somme de tone440 + bypass inversé
sox -m -v 1 /root/tests/in/tone440.wav -v -1 /root/tests/out/bypass.wav \
    /root/tests/out/null.wav
sox /root/tests/out/null.wav -n stat 2>&1 | grep 'RMS.*amplitude'
\end{lstlisting}
\textbf{Pass si} : RMS de null.wav $<$ -140~dBFS (bruit de quantification numérique). \textbf{Fail} si $>$ -80~dBFS.
...
sleep 0.1
aplay -D plychw:softac5212tdm,Source_Play /root/tests/in/click.wav
wait
...
\end{lstlisting}
\textbf{Pass si} : script Python reporte delay $\leq$ 4{,}2~ms (marge 5\%).
...
\end{lstlisting}
\textbf{Pass si} : utilisation rapportée $<$ 30\% de la capacité HiFi4.
...
\texttt{meta-local/recipes-core/images/imx-image-full.bbappend} & \texttt{IMAGE\_INSTALL += tac5212-init sof-presets phase1a-tests} \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item Build topology (m4 + alsatplg), deploy, reboot.
  ...
  \item Exécuter T4 (null test bypass).

...
\paragraph{\texttt{sof-imx8mp-tac5212-pipe-masterchain.m4} V2.4}
Pipeline d'entrée de la chaîne master, calqué sur \texttt{pipe-volume-playback.m4} (W\_PGA), \texttt{pipe-multiband-drc-playback.m4} (mbdrc), et \texttt{pipe-amp-ref-capture.m4} (W\_MUXDEMUX comme tap cross-pipeline). Valeurs : 2 canaux, s32le, 96 frames/period (2 ms @ 48 kHz). \textbf{[V2.4]} Ajout du mux demux après le DRC pour exporter le signal vers PIPE 5 Master\_Tap via un lien DAPM widget-to-buffer (autorisé par IPC3).

\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
# Master chain input V2.4 : host PCM -> mbdrc -> pga -> drc -> muxdemux -> (2 sinks)
# Patterns: pipe-volume-playback.m4 (PGA) + pipe-multiband-drc-playback.m4 (mbdrc)
#        + pipe-amp-ref-capture.m4 (muxdemux cross-pipeline tap)
...
W_BUFFER(3, COMP_BUFFER_SIZE(2,
    ...
    COMP_PERIOD_FRAMES(PCM_MAX_RATE, SCHEDULE_PERIOD)),
    PLATFORM_COMP_MEM_CAP)
# [V2.4] B4 = demux out monitor, B5 = demux out tap (vers PIPE 5)
...
# Graph : host -> B0 -> mbdrc -> B1 -> pga -> B2 -> drc -> B3 -> demux -> B4+B5
...
# [V2.4] Export buffer 4 comme SOURCE monitor de la pipeline
# (B5 est consomme par PIPE 5 via demux widget cross-pipeline)
...
# [V2.4] Export MUXDEMUX widget pour lien cross-pipeline depuis le top-level
# Pattern: sof/pipe-amp-ref-capture.m4:109

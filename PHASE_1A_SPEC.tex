...
\texttt{ROUTE\_MATRIX(stream\_id, mask\_ch0, mask\_ch1, ...)} : chaque byte est un mask. \texttt{1} = out\_ch0, \texttt{2} = out\_ch1. Donc ch0 entrée → ch0 sortie, ch1 entrée → ch1 sortie. Duplication pure stéréo.

Le stream\_id = PIPELINE\_ID = 3 (pipeline source) par convention upstream dans \texttt{pipe-amp-ref-capture.m4}.

...
\paragraph{\texttt{sof-imx8mp-tac5212-pipe-masterchain.m4} V2.1}
Pipeline d'entrée de la chaîne master. Valeurs concrètes : 2 canaux, s32le, 96 frames/period (2 ms @ 48 kHz). \textbf{[V2.1]} Utilise \texttt{W\_PGA} (pas \texttt{W\_VOLUME}) + \texttt{C\_CONTROLMIXER} pour exposer "Master Volume". Exporte \texttt{PIPELINE\_SCHED\_COMP\_<id>} + appelle \texttt{W\_PIPELINE(...)}.

\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
# Master chain input V2.1: host PCM -> mbdrc -> pga -> drc -> buf
...
include(`pga.m4')             # [V2.1] W_PGA pas W_VOLUME
include(`drc.m4')
include(`common/tlv.m4')      # [V2.1] pour TLV_DB_SCALE
...
# --- PGA (Programmable Gain Amplifier = volume DVC) [V2.1] ---
# W_PGA signature: (index, format, periods_sink, periods_source, kcontrol_list, core)
define(PGA_NAME, `Master Volume')
C_CONTROLMIXER(Master Volume, PIPELINE_ID,
    CONTROLMIXER_OPS(volsw, 256 binds mixer control to volume get/put, 256, 256),
    CONTROLMIXER_MAX(, 32),
    false,
    CONTROLMIXER_TLV(TLV_DB_SCALE_WTYPE(vtlv_m64s2, -6400, 50, 1), vtlv_m64s2),
...
# --- Buffers : host | mbdrc-out | pga-out | drc-out(shared) ---
...
# [V2.1] Export SCHED_COMP pour permettre aux autres pipelines
# (ex: PIPE 5 master tap) de piggyback sur notre scheduler
indir(`define', concat(`PIPELINE_SCHED_COMP_', PIPELINE_ID),
      N_PCMP(PCM_ID))

# [V2.1] Widget scheduler explicite

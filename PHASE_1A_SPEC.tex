...
% =======================================================================
\section*{Changelog V2.5 $\rightarrow$ V2.6}
\addcontentsline{toc}{section}{Changelog V2.5 $\rightarrow$ V2.6}

Investigation V2.5 (job \texttt{cdd3c7ab}) : 1 seul vrai bug critique trouvé en consensus (4 workers sur 5) --- le reste est faux positif dû au fait que les modifications SOF n'étaient pas pushées dans le fork.

\begin{center}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Ref} & \textbf{Sévérité} & \textbf{Correction V2.6} \\
\midrule
B1 & Critique & \texttt{ROUTE\_MATRIX(PIPELINE\_ID, ...)} $\rightarrow$ \texttt{ROUTE\_MATRIX(5, ...)}. Le 1\textsuperscript{er} arg de \texttt{ROUTE\_MATRIX} est le \texttt{stream\_id} = \texttt{pipeline\_id} du SINK (Master\_Tap = PIPE 5), pas celui de la pipeline hôte du mux (PIPE 3). Raison : \texttt{demux\_process} dans \texttt{mux.c:248-260} appelle \texttt{get\_stream\_index(sink->pipeline\_id)} au runtime et cherche \texttt{streams[].pipeline\_id == 5}. Avec \texttt{3}, aucun match $\rightarrow$ retour \texttt{-EINVAL} et data non routée. Pattern identique à \texttt{pipe-amp-ref-capture.m4:53-61} qui utilise \texttt{SMART\_PB\_PPL\_ID} (ID du SINK). \\
Fork push & Prérequis & Les modifications V2.3+ (board config CONFIG\_COMP\_MUX=y, fichiers \texttt{pipe-masterchain.m4}, \texttt{pipe-mastertap.m4}, \texttt{masterlite.m4}, \texttt{demux\_route\_default.m4}) doivent être \textbf{commitées et pushées} dans le fork \texttt{Mjxkill/sof} branche \texttt{feature/sdma-ap2ap-phase1} pour que les investigations futures voient l'état réel. Les faux positifs V2.5 "CONFIG\_COMP\_MUX manquant" / "fichiers masterchain manquants" viennent de cette absence de push. \\
\bottomrule
\end{tabularx}
\end{center}

\subsubsection*{Faux positifs V2.5 écartés}

\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item \textbf{"CONFIG\_COMP\_MUX=y manquant dans imx8mp\_evk\_mimx8ml8\_adsp.conf"} (gemini, claude-code) : FAUX. La modif est locale, non pushée. Le fichier contient bien la ligne chez nous.
  \item \textbf{"Fichiers pipe-masterchain.m4 / pipe-mastertap.m4 / masterlite.m4 / demux\_route\_default.m4 manquants"} : FAUX. Ils existent localement. À pusher.
  \item \textbf{"Pipeline sans sink DAPM local risque scheduling"} (gemini, claude-code) : risque THÉORIQUE. Pas de preuve kernel de crash. À valider empiriquement lors du load tplg.
  \item \textbf{"DAPM direction inversée dans SectionGraph"} (glm-5.1) : FAUX. Le pattern \texttt{dapm(sink, source)} est correct (cf. \texttt{utils.m4:36} + 4 patterns upstream). V2.5 écrit \texttt{dapm(PIPELINE\_SINK\_5, PIPELINE\_DEMUX\_3)} = sink=PIPE5.B0, source=MUXDEMUX3.0 $\rightarrow$ le signal va du mux vers le buffer PIPE 5. Correct.
\end{itemize}

Architecture V2.4 (mux demux cross-pipeline) + correction squelette V2.5 (K6/K7/K8) + fix pipeline\_id V2.6 = squelette final prêt pour test empirique \texttt{m4 | alsatplg | kernel load}.

\clearpage

% =======================================================================
\section*{Changelog V2.4 $\rightarrow$ V2.5}
\addcontentsline{toc}{section}{Changelog V2.4 $\rightarrow$ V2.5}

V2.4 corrigeait l'erreur kernel \texttt{-22} via \texttt{W\_MUXDEMUX} cross-pipeline (pattern architecturalement confirmé par 5 workers sur 5). Mais l'investigation ffae75ee a trouvé \textbf{3 bugs d'exécution} dans le squelette V2.4 :

\begin{center}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Ref} & \textbf{Sévérité} & \textbf{Correction V2.5} \\
\midrule
K6 & Critique & \texttt{demux\_route\_default.m4} n'existe pas dans le fork. V2.5 crée le fichier blob dans \texttt{meta-local/sof-presets/} (\texttt{sof\_mux\_config} minimal : 1 stream, route 2ch identité). \\
K7 & Critique & Double sink même pipeline : V2.4 avait B4 ET B5 sinks du mux dans PIPE 3. \texttt{demux\_process} (\texttt{mux.c:248-260}) démultiplexe par \texttt{pipeline\_id} ; 2 buffers même pipeline écrasent \texttt{sinks\_stream[]}. V2.5 supprime B4 --- le mux n'a plus qu'1 sink cross-pipeline (B5 de PIPE 5). \\
K8 & Critique & Buffers terminaux sans consommateur : B4 same-pipeline sans downstream → back-pressure fatale (\texttt{demux\_trigger} n'active \texttt{overrun\_permitted=true} QUE pour sinks cross-pipeline, \texttt{mux.c:453}). V2.5 : seul B5 cross-pipeline subsiste, overrun OK. \\
Scope & Info & \textbf{Implication fonctionnelle} : en Phase 1a.1 MVP, le signal master-processé sort uniquement vers \texttt{Master\_Tap} (pour NPU). Le monitor master$\rightarrow$HP arrive en Phase 1a.3 (mux merge avec SAI\_Playback 8ch). Cohérent avec l'incrément initial : 1a.1 = master chain + tap, 1a.3 = merge vers SAI. \\
\bottomrule
\end{tabularx}
\end{center}
...
% =======================================================================
\section*{Changelog V2.3 $\rightarrow$ V2.4}
\addcontentsline{toc}{section}{Changelog V2.3 $\rightarrow$ V2.4}

V2.3 passait \texttt{m4 | alsatplg} mais le \textbf{kernel rejetait au load} avec :
\begin{verbatim}
sof_ipc3_route_setup: route BUF3.3 -> BUF5.0 failed (-22)
\end{verbatim}

\textbf{Cause racine} : SOF IPC3 interdit les liens DAPM entre 2 \texttt{W\_BUFFER} de pipelines différentes. Les buffers SOF sont mono-consommateur, ils ne peuvent pas être "sources" pour une autre pipeline. Le risque N3 flaggé en V2.1/V2.2/V2.3 s'est matérialisé.

\textbf{Correction V2.4} : introduction d'un composant \texttt{W\_MUXDEMUX} (mux en mode demux) à la fin de la master chain de PIPE 3. Ce composant a la capability multi-sink DAPM cross-pipeline (contrairement aux buffers). Le lien devient \texttt{dapm(BUF, WIDGET\_MUX)} au lieu de \texttt{dapm(BUF, BUF)} --- autorisé par IPC3.

\begin{center}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Ref} & \textbf{Sévérité} & \textbf{Correction V2.4} \\
\midrule
K1 & Critique & Ajout \texttt{W\_MUXDEMUX(0, 1, PIPELINE\_FORMAT, 2, 2, SCHEDULE\_CORE, LIST(...))} après le DRC dans \texttt{pipe-masterchain.m4}. Mode demux (\$2=1) avec 1 input et 2 sinks (B4 monitor + B5 vers tap). Pattern identique à \texttt{sof/pipe-amp-ref-capture.m4:80} (use case echo-reference). \\
K2 & Critique & Ajout \texttt{indir('define', PIPELINE\_DEMUX\_<id>, N\_MUXDEMUX(0))} pour exporter le widget mux demux vers le top-level, permettant le \texttt{dapm} cross-pipeline. Pattern identique à \texttt{pipe-amp-ref-capture.m4:109}. \\
K3 & Critique & Dans \texttt{masterlite.m4}, changement du SectionGraph : \texttt{dapm(PIPELINE\_SINK\_5, PIPELINE\_SOURCE\_3)} devient \texttt{dapm(PIPELINE\_SINK\_5, PIPELINE\_DEMUX\_3)}. Le lien pointe vers un WIDGET (le mux) et non un buffer --- IPC3 l'autorise. Pattern prouvé par \texttt{sof-smart-amplifier.m4:204} et \texttt{sof-mt8195-mt6359-rt1019-rt5682.m4:130}. \\
K4 & Medium & Ajout \texttt{C\_CONTROLBYTES(DEMUX, ...)} avec blob \texttt{sof\_mux\_config} généré par \texttt{ROUTE\_MATRIX} macro de \texttt{muxdemux.m4}. Route identique sur les 2 sinks (tap = copie pure du signal master post-DRC). \\
K5 & Medium & Ajout \texttt{include('muxdemux.m4')} dans pipe-masterchain.m4. Composant \texttt{CONFIG\_COMP\_MUX=y} déjà activé dans board config V2.3. \\
Latency & Info & Master\_Tap latency passe de 8 ms à 10 ms (+1 cycle DSP via le mux). Acceptable car NPU update rate = 10 Hz ($\approx$100 ms de cycle), +2 ms insignifiant. Chemin principal mic$\rightarrow$HP (PIPE 2) reste à 4 ms --- non affecté. \\
\bottomrule
\end{tabularx}
\end{center}

\subsubsection*{Preuves upstream du pattern (fork Mjxkill/sof feature/sdma-ap2ap-phase1)}

\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item \texttt{sof/tools/topology/topology1/m4/muxdemux.m4} : définit \texttt{W\_MUXDEMUX}, \texttt{N\_MUXDEMUX}, \texttt{ROUTE\_MATRIX}, UUIDs mux/demux.
  \item \texttt{sof/tools/topology/topology1/sof/pipe-amp-ref-capture.m4} : use case \textbf{identique} au nôtre --- capture avec demux pour envoyer à la fois user space ET une autre pipeline FW. Ligne 3 docstring : "capture with demux for both echo reference to user space and the feedback to smart amplifier FW algorithm".
  \item \texttt{sof/tools/topology/topology1/sof-smart-amplifier.m4:204} : pattern top-level \texttt{dapm(N\_SMART\_REF\_BUF, N\_SMART\_DEMUX)} --- buffer d'une pipeline vers mux widget d'une autre pipeline. Fonctionne upstream.
  \item \texttt{sof/tools/topology/topology1/sof-mt8195-mt6359-rt1019-rt5682.m4:130} : même pattern \texttt{dapm(N\_AEC\_REF\_BUF, PIPELINE\_DEMUX\_1)}.
\end{itemize}

\subsubsection*{Validation empirique prévue}

V2.4 est validée par \textbf{test de chargement kernel} (pas seulement par m4+alsatplg). Critère de succès : \texttt{dmesg | grep sof\_ipc3\_route\_setup} ne montre pas d'erreur \texttt{-22} après deploy du nouveau tplg.

\clearpage

% =======================================================================
\section*{Changelog V2.2 $\rightarrow$ V2.3}
\addcontentsline{toc}{section}{Changelog V2.2 $\rightarrow$ V2.3}

V2.3 intègre la \textbf{4\textsuperscript{e} investigation critic} (job \texttt{1986cc65}). Verdict global : \textbf{les 7 corrections V2.1→V2.2 sur le fond sont toutes confirmées correctes}, mais 3 bugs critiques de \emph{transposition} étaient restés dans le squelette \texttt{pipe-masterchain.m4} (noms de fichiers d'include et de macros raccourcis à tort). V2.3 applique le diff minimal.

\begin{center}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Ref} & \textbf{Sévérité} & \textbf{Correction V2.3} \\
\midrule
L1 & Critique & \texttt{include(`mbdrc\_coef\_default.m4')} $\rightarrow$ \texttt{include(`multiband\_drc\_coef\_default.m4')}. Le fichier coef réel a le nom complet. \\
L2 & Critique & \texttt{define(MBDRC\_priv, ...)} $\rightarrow$ \texttt{define(MULTIBAND\_DRC\_priv, ...)}. Le fichier \texttt{multiband\_drc\_coef\_default.m4:2} contient \texttt{CONTROLBYTES\_PRIV(MULTIBAND\_DRC\_priv, ...)} en littéral ; sans ce nom exact, \texttt{alsatplg} ne trouve pas la SectionData. Pattern identique dans \texttt{sof/pipe-multiband-drc-playback.m4:32}. \\
L3 & Critique & \texttt{include(`drc\_coef\_limiter\_default.m4')} $\rightarrow$ \texttt{include(`drc\_coef\_default.m4')}. Fichiers existants dans le fork : \texttt{drc\_coef\_default.m4} (DRC générique) et \texttt{drc\_coef\_speaker\_default.m4}. Aucun "limiter\_default". Le blob est tunable à chaud via \texttt{Master Limiter Config} pour atteindre un comportement limiter. \\
N1 & Medium & Annexe : \texttt{VOL\_ZERO\_DB = 0x40000000} (Q2.30, stale V1) $\rightarrow$ \texttt{0x10000} (Q8.16 IPC3), cohérent avec le changelog V2.1. \\
Polish & Low & Renommage \texttt{MBDRC\_CTRL} $\rightarrow$ \texttt{MY\_MULTIBAND\_DRC\_CTRL} et \texttt{DRC\_CTRL} $\rightarrow$ \texttt{MY\_DRC\_CTRL} (convention \texttt{MY\_} upstream pour éviter collision macro). \texttt{undefine} mis à jour. \\
\bottomrule
\end{tabularx}
\end{center}

\textbf{Bugs medium connus et assumés} (pas corrigés en V2.3, à traiter plus tard) :
\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item \textbf{N2} : les switches \texttt{Master MBDRC Bypass} / \texttt{Master Limiter Bypass} référencés par T4 ne sont pas déclarés dans la topology. Le bypass sera piloté via \texttt{sof-ctl} en poussant un blob \texttt{process\_enabled=0} plutôt que par un switch ALSA. T4 adapté en conséquence.
  \item \textbf{N3} : \texttt{PIPELINE\_SCHED\_COMP\_<id> = N\_PCMP(...)} (vs \texttt{N\_DAI\_IN} upstream) --- fonctionnel mais non-canonique. Test runtime requis en Séquence 1a.1.
  \item \textbf{P2} : \texttt{W\_BUFFER} en 3 args (pas de \texttt{SCHEDULE\_CORE}) --- conforme au pattern \texttt{pipe-eq-iir-volume-playback.m4}, i.MX8MP est mono-core HiFi4 donc sans impact. Acceptable.
\end{itemize}

Après V2.3, le squelette m4 doit pouvoir passer \texttt{m4 \textbar\ alsatplg} sans erreur. C'est le prochain gate de validation.

\clearpage

% =======================================================================
\section*{Changelog V2.1 $\rightarrow$ V2.2}
\addcontentsline{toc}{section}{Changelog V2.1 $\rightarrow$ V2.2}

V2.2 intègre la \textbf{3\textsuperscript{e} investigation critic} (job \texttt{800fdc1f}) qui a remonté \textbf{7 bugs de syntaxe m4} dans le squelette V2.1 --- tous des erreurs de signature de macro ou de quotage. Le pattern canonique retenu pour référence est \texttt{sof/tools/topology/topology1/sof/pipe-volume-playback.m4} (qui compile et fonctionne upstream).

\begin{center}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Ref} & \textbf{Sévérité} & \textbf{Correction V2.2} \\
\midrule
C1-NEW & Critique & Signature \texttt{W\_PGA} réelle (\texttt{m4/pga.m4:10-55}) = 7 args : \texttt{(index, format, periods\_sink, periods\_source, data\_config, core, kcontrol\_list)}. V2.1 n'en passait que 6. V2.2 ajoute le prélude canonique \texttt{W\_VENDORTUPLES(DEF\_PGA\_TOKENS,...)} + \texttt{W\_DATA(DEF\_PGA\_CONF, DEF\_PGA\_TOKENS)} et appelle W\_PGA avec DEF\_PGA\_CONF en \$5. \\
C2-NEW & Critique & \texttt{define(PGA\_NAME, 'Master Volume')} faisait que \texttt{SectionWidget} se nommait "Master Volume" au lieu de \texttt{PGA3.0}, cassant le DAPM qui référence \texttt{N\_PGA(0)} $\rightarrow$ \texttt{PGA3.0}. V2.2 supprime ce \texttt{define} (pattern \texttt{pipe-volume-playback.m4} n'en utilise pas). \\
C3-NEW & Critique & \texttt{TLV\_DB\_SCALE\_WTYPE(...)} est une macro inexistante (\texttt{grep -r TLV\_DB\_SCALE sof/} = 0 match). V2.2 utilise un commentaire libre \texttt{TLV 32 steps from -64dB to 0dB for 2dB}, conforme au pattern upstream. \\
H1-NEW & Haute & Référence kcontrol \texttt{"PGA\_NAME"} ne matchait pas le nom \texttt{SectionControlMixer} créé par C\_CONTROLMIXER (\texttt{"<PIPELINE\_ID> Master Volume"}). V2.2 utilise \texttt{"PIPELINE\_ID Master Volume"}. \\
H2-NEW & Haute & \texttt{PIPELINE\_PCM\_ADD} pour PIPE 3 n'avait que 11 args $\rightarrow$ \texttt{SCHEDULE\_TIME\_DOMAIN} devenait chaîne vide (pipeline.m4:82 fait \texttt{define(SCHEDULE\_TIME\_DOMAIN, \$12)}). V2.2 ajoute \texttt{SCHEDULE\_TIME\_DOMAIN\_TIMER} comme 12\textsuperscript{e} arg. \\
H3-NEW & Haute & SectionGraph top-level avec \texttt{`dapm(...)'} (backticks) empêchait l'expansion macro. V2.2 enlève les backticks autour de \texttt{dapm()} (pattern \texttt{sof-imx8mp-wm8960-kwd.m4:75-83}). \\
H4 & Haute & Réconciliation changelog $\leftrightarrow$ code : V2.2 confirme que \textbf{PIPE 3 a son propre W\_PIPELINE} (scheduler autonome TIMER) et \textbf{PIPE 5 piggyback sur PIPELINE\_SCHED\_COMP\_3}. Le changelog V2.1 mentionnant "piggyback sur SCHED\_COMP\_1" était incohérent ; la bonne approche est celle du code. \\
\bottomrule
\end{tabularx}
\end{center}

\textbf{Faux positif écarté} : gemini-3-pro a prétendu \texttt{VOL\_ZERO\_DB = 0x800000} en IPC3. Vérification directe (\texttt{sof/src/audio/volume/volume.h:41-63,103}) : en IPC3, \texttt{VOL\_QXY\_Y = 16} $\rightarrow$ \texttt{BIT(16) = 0x10000}. Les 4 autres workers (kimi, claude-code, minimax, glm-5.1) ont tous confirmé 0x10000. V2.1 avait raison, pas besoin de modifier.

\clearpage

% =======================================================================
\section*{Changelog V2 $\rightarrow$ V2.1}
\addcontentsline{toc}{section}{Changelog V2 $\rightarrow$ V2.1}

Cette V2.1 intègre une \textbf{2\textsuperscript{e} investigation critic} (job \texttt{e9c99a99}) qui, pour la première fois, avait accès au fork SOF réel (\texttt{Mjxkill/sof} branche \texttt{feature/sdma-ap2ap-phase1}) via le paramètre \texttt{sub\_repos} du MCP. Les claims de la V2 ont été vérifiés contre les vraies sources et 3 bugs critiques + 3 high bloquaient le build.

\begin{center}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Ref} & \textbf{Sévérité} & \textbf{Correction V2.1} \\
\midrule
C1 & Critique & \texttt{W\_VOLUME} n'existe pas, \texttt{SOF\_VOLUME\_RAMP\_NONE} non plus. Remplacé par \texttt{W\_PGA} (macro réelle, \texttt{m4/pga.m4:11}) + \texttt{C\_CONTROLMIXER} avec TLV pour exposer "Master Volume". \\
C2 & Critique & Squelette \texttt{pipe-masterchain.m4} ajoute \texttt{indir(define, PIPELINE\_SCHED\_COMP\_<id>, N\_PCMP(PCM\_ID))} + \texttt{W\_PIPELINE(...)}. \texttt{pipe-mastertap.m4} ajoute aussi \texttt{W\_PIPELINE(SCHED\_COMP, ...)}. Sans ces lignes, le scheduler est orphelin. \\
C3 & Critique & \texttt{drc2ms.m4} n'existe pas dans le fork (seul \texttt{drc8ms.m4} est commité). V2.1 change la baseline référencée à \texttt{sof-imx8mp-tac5212-drc8ms.m4} ; le fichier \texttt{drc2ms.m4} local (déployé en session précédente) sera commité AVANT l'implémentation. \\
H1 & Haute & \texttt{sof/app/boards/imx8mp\_evk\_mimx8ml8\_adsp.conf} reçoit \texttt{CONFIG\_COMP\_MULTIBAND\_DRC=y}, \texttt{CONFIG\_COMP\_MUX=y}, \texttt{CONFIG\_COMP\_VOLUME=y} en plus du \texttt{CONFIG\_COMP\_DRC=y} existant. \\
H2 & Haute & Correction : \texttt{VOL\_ZERO\_DB = 0x10000} (Q8.16 en IPC3), pas \texttt{0x40000000} (Q2.30). Toute la doc V2 qui citait \texttt{0x40000000} est reprise. \\
H3 & Haute & PIPE 3 (master chain) et PIPE 5 (tap) piggyback \textbf{tous deux} sur \texttt{PIPELINE\_SCHED\_COMP\_1} (DMA-driven SAI RX). Garantit un scheduling commun avec PIPE 2 (monitor) quand Phase 1a.3 ajoute le mux merge. \\
\bottomrule
\end{tabularx}
\end{center}

\textit{Les verdicts CONFIRMÉS de V2 (bypass bit-perfect multiband\_drc/drc, AP2AP memcpy\_dma, domain\_is\_pending, zephyr\_ll hook, PCM\_CAPTURE\_ADD/PCM\_PLAYBACK\_ADD, args PIPELINE\_PCM\_ADD, SDMA3 32 canaux) restent valides et ne sont pas re-listés ici.}
...
\subsection{Baseline existante --- point de départ (V2.1)}
\textbf{La topology actuellement commitée dans le fork SOF fonctionne en duplex}. Rappel :
\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item Fichier de référence commité : \texttt{sof-imx8mp-tac5212-drc8ms.m4} (variante 8 ms, dans le fork \texttt{feature/sdma-ap2ap-phase1})
  \item Variante locale 2 ms non-commitée : \texttt{sof-imx8mp-tac5212-drc2ms.m4} créée en session précédente par \texttt{cp drc4ms.m4 drc2ms.m4} + edit de la période (4000$\rightarrow$2000). Déployée manuellement via \texttt{scp .tplg} sur la board.
...
\end{itemize}
\textbf{[V2.1]} Avant toute extension en Phase 1a.1, \textbf{commiter \texttt{drc2ms.m4}} dans le fork SOF (ou l'ignorer et repartir de \texttt{drc8ms.m4}). Sans commit, les workers et les futurs ré-builds ne verront pas le fichier.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item \texttt{volume} à 0 dB (gain \texttt{VOL\_ZERO\_DB = 0x10000} en Q8.16 pour IPC3) $\rightarrow$ bascule en mode \texttt{is\_passthrough=true} (volume.c:349-358) $\rightarrow$ fonction \texttt{vol\_passthrough\_s32\_to\_s24\_s32} (volume\_hifi4.c:287-324) utilise AE\_LA32X2/AE\_SA32X2 SIMD pur, sans multiplication $\rightarrow$ bit-perfect garanti. \textbf{[V2.1 correction]} La V2 citait à tort \texttt{0x40000000} (Q2.30).
\end{itemize}
...
\paragraph{\texttt{sof-imx8mp-tac5212-pipe-masterchain.m4} V2.5}
Pipeline d'entrée de la chaîne master. Valeurs : 2 canaux, s32le, 96 frames/period (2 ms @ 48 kHz). \textbf{[V2.5]} MUXDEMUX a 1 seul sink cross-pipeline (via export \texttt{PIPELINE\_DEMUX\_3}) --- pas de buffer local B4 (supprimé pour éviter K7/K8). Le monitor master$\rightarrow$HP local est reporté à Phase 1a.3.

\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
# Master chain input V2.5 : host PCM -> mbdrc -> pga -> drc -> muxdemux -> (cross-pipe B5)
...
\paragraph{\texttt{sof-imx8mp-tac5212-pipe-mastertap.m4} V2.1}
Pipeline de sortie côté Master\_Tap, simple capture depuis un buffer externe (partagé avec pipe-masterchain buf\#3 via DAPM). \textbf{[V2.1]} Ajout du widget scheduler \texttt{W\_PIPELINE(SCHED\_COMP,...)} qui piggyback sur le scheduler de l'upstream (PIPELINE\_SCHED\_COMP\_3 reçu en argument).
\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
...
# (SCHED_COMP est defini par le sof-imx8mp-tac5212-masterlite.m4 appelant)
...
\subsection{Board config SOF à modifier (V2.1, H1)}

Le fichier \texttt{sof/app/boards/imx8mp\_evk\_mimx8ml8\_adsp.conf} ne contient actuellement que \texttt{CONFIG\_COMP\_DRC=y}. Phase 1a nécessite d'activer plusieurs composants additionnels :
...
\textbf{Impact} : cette modif force un \textbf{rebuild firmware SOF complet} (west build + rimage + cat xman + scp \texttt{sof-imx8m.ri}). Pas juste un rebuild topology. Prévoir 10-15 min supplémentaires la première fois.

\subsection{Valeur VOL\_ZERO\_DB (V2.1, H2)}

La V2 citait \texttt{0x40000000} (Q2.30). \textbf{C'est faux en IPC3}. Valeurs correctes :

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{IPC} & \textbf{Format} & \textbf{VOL\_ZERO\_DB (hex)} \\
\midrule
IPC3 & Q8.16 & \texttt{0x10000} = 65\,536 \\
IPC4 & Q1.23 & \texttt{0x800000} = 8\,388\,608 \\
\bottomrule
\end{tabular}
\end{center}

Définition dans \texttt{sof/src/audio/volume/volume.h:103} : \texttt{VOL\_ZERO\_DB = BIT(VOL\_QXY\_Y)}. Sur IPC3 (notre cas), \texttt{VOL\_QXY\_Y = 16} donc \texttt{BIT(16) = 0x10000}.

Tout preset \texttt{bytes blob} qui force "0 dB" doit utiliser \texttt{0x10000}, pas \texttt{0x40000000}. Les scripts de test qui appellent \texttt{amixer cset name='Master Volume' 0dB} s'en fichent (la macro dB $\rightarrow$ Q8.16 est dans le kernel), mais les blobs binaires bruts doivent être conformes.

\subsection{Fichier blob demux à créer (V2.5, K6)}

Le squelette V2.5 inclut \texttt{demux\_route\_default.m4} qui n'existe pas upstream. À créer en local dans \texttt{sof/tools/topology/topology1/m4/} (ou dans \texttt{meta-local/recipes-kernel/linux/files/} si on veut éviter de toucher le fork). Contenu minimal (1 stream, route 2ch identité) :
...
# Raison : demux_process(mux.c:248-260) fait
#   get_stream_index(sink->pipeline_id) -> cherche streams[].pipeline_id == 5
...
\texttt{ROUTE\_MATRIX(stream\_id, mask\_ch0, mask\_ch1, ..., mask\_ch7)} : 9 args. Le \texttt{stream\_id} est le \texttt{pipeline\_id} du buffer sink connecté. Les 8 masks sont bit-fields d'output channels (\texttt{1} = out\_ch0, \texttt{2} = out\_ch1, \texttt{4} = out\_ch2, etc.).

Configuration retenue : ch0 entrée → ch0 sortie (mask=1), ch1 entrée → ch1 sortie (mask=2), autres canaux inutilisés (mask=0). Duplication stéréo pure vers PIPE 5.

Pattern upstream : \texttt{pipe-amp-ref-capture.m4:53-61} utilise \texttt{SMART\_PB\_PPL\_ID} (ID de la pipeline playback sink), pas \texttt{PIPELINE\_ID} (ID de la capture).
...
\texttt{volume} & \texttt{volume.c} & gain Q8.16 $=$ \texttt{0x10000} $=$ 1{,}0 (IPC3) $\Rightarrow$ bascule \texttt{is\_passthrough=true} $\rightarrow$ \texttt{vol\_passthrough} SIMD sans mult [V2.3 corr.] \\
\bottomrule
\end{tabularx}
\end{center}

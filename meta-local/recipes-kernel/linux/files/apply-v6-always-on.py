...
if helper_marker not in text:
    ...
    helper_code = (
        ...
        " * Sends SOF_IPC_TPLG_PIPE_TRIGGER with the firmware COMP_TRIGGER_* enum value.\n"
        " * cmd values must match firmware src/include/sof/audio/component.h:\n"
        ...
        "\tmsg.pipeline_id = pipeline_id;\n"
        "\tmsg.cmd = cmd;\n"
        "\n"
        "\tdev_dbg(sdev->dev, \"V6.0: pipe_trigger pipeline %u cmd %u\\n\",\n"
        "\t\tpipeline_id, cmd);\n"
        "\tret = sof_ipc_tx_message_no_reply(sdev->ipc, &msg, sizeof(msg));\n"
        "\tif (ret < 0)\n"
        "\t\tdev_err(sdev->dev, \"V6.0: pipe_trigger pipeline %u cmd %u failed: %d\\n\",\n"
        "\t\t\tpipeline_id, cmd, ret);\n"
        "\treturn ret;\n"
        "}\n"
        "\n"
...
if trigger_marker not in text:
    ...
    trigger_block = (
        ...
        "\t * setup, DAI_CONFIG sent, and PIPE_COMPLETE acked.\n"
        "\t */\n"
        "\tif (!verify) {\n"
        ...
        "\t\t\t\tcontinue;\n"
        "\t\t\tret = sof_ipc3_send_pipe_trigger(sdev, aw->pipeline_id,\n"
        ...
        "\t\t\tif (ret < 0)\n"
        "\t\t\t\treturn ret;\n"
        "\t\t\tdev_info(sdev->dev, \"V6.0: pipeline %d started (always-on)\\n\",\n"
        "\t\t\t\t aw->pipeline_id);\n"
        "\t\t}\n"
        "\t}\n"
    )

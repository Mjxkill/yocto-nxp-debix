...
\title{\textbf{Plateforme Mastering Live Debix}\\[6pt]
  \Large Spécification Phase 1a --- \textbf{V4.2}\\[4pt]
  \large Console numérique 8 canaux end-to-end\\
  \normalsize Intègre investigations V4 (854s) + V4.1 (790s)}
\author{Document de référence technique}
\date{2026-04-22 --- rev V4.2}
...
% =======================================================================
\section{Ce qui change depuis V4.1}

V4.1 a été soumis à l'investigation finale (6 workers : gemini 114s, kimi 363s, glm 438s, minimax 633s, claude-code 643s, qwen 785s). \textbf{Verdict unanime : GO pour Phase 1a.1 MVP} --- le squelette passe la gate \texttt{m4 | alsatplg}. V4.2 intègre les micro-fixes recommandés et clarifie la stratégie T5b.

\subsection{Micro-fixes V4.2}

\begin{center}
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Fix} & \textbf{Correction V4.2} \\
\midrule
F1 & \texttt{CONFIG\_DRC\_MAX\_PRE\_DELAY\_FRAMES = 512} (et non 256 comme dit V4.1 \S5). Pre-delay par défaut = \texttt{DRC\_DEFAULT\_PRE\_DELAY\_FRAMES = 256} qui correspond à \texttt{CONFIG\_DRC\_MAX\_PRE\_DELAY\_FRAMES >> 1}. \\
F2 & Ajout 12e arg \texttt{SCHEDULE\_TIME\_DOMAIN\_DMA} à \texttt{PIPELINE\_PCM\_ADD} pour hygiène et cohérence avec \texttt{DAI\_ADD} (claude-code, pattern masterlite). \\
F3 & Stratégie T5b formalisée : topology variante \texttt{masterV42-T5.m4} qui OMET le widget \texttt{W\_DRC} de la chaîne pour le null test bit-perfect strict. C'est la \textbf{seule voie sample-exact} car \texttt{drc\_s32\_default} applique toujours le pre-delay même si \texttt{enabled=0}. \\
F4 & Note CMakeLists : GLOB \texttt{sof/*.m4} (ligne 7 de \texttt{CMakeLists.txt}) capture automatiquement les fichiers pipeline, mais le top-level \texttt{masterV42.m4} doit être ajouté explicitement à \texttt{TPLGS} pour build standalone. La recipe Yocto \texttt{meta-local} peut aussi le compiler via invocation \texttt{m4 | alsatplg} directe (pattern baseline). \\
F5 & Note DAI\_ADD : 12 args acceptés (pattern \texttt{DAI\_ADD\_SCHED} qui passe 12 args). Les 3 args manquants (\texttt{DAI\_CHANNELS}, \texttt{DAI\_RATE}, \texttt{DYNAMIC\_PIPE}) sont inutilisés par \texttt{pipe-dai-capture.m4}/\texttt{pipe-dai-playback.m4}. Pas de modification nécessaire. \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{Ce qui est confirmé unanimement par les 6 workers V4.1}

\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item \textbf{Gate compile} : le squelette pass \texttt{m4 | alsatplg}
  \item \texttt{multiband\_drc\_coef\_passthrough.m4} \textbf{existe} dans notre fork (70 lignes, 2 bands, \texttt{enable\_emp\_deemp=0})
  \item \texttt{W\_PIPELINE} créé UNIQUEMENT par \texttt{pipe-dai-\{capture,playback\}.m4} via \texttt{DAI\_ADD} --- un seul scheduler widget par pipeline\_id
  \item Pattern kcontrol \texttt{"PIPELINE\_ID Master Capture Volume"} \textbf{correct} : m4 expand \texttt{PIPELINE\_ID} dans les strings double-quotées
  \item \texttt{W\_MULTIBAND\_DRC} / \texttt{W\_DRC} en \texttt{(2, 2)} confirmé par upstream
  \item \texttt{W\_PGA} capture \texttt{(2, DAI\_PERIODS)}, playback \texttt{(DAI\_PERIODS, 2)}
  \item Exports \texttt{PIPELINE\_SCHED\_COMP\_<id>} (capture) et \texttt{PIPELINE\_PLAYBACK\_SCHED\_COMP\_<id>} (playback) disponibles pour piggyback Phase 1a.2
\end{itemize}
...
\subsection{\texttt{sof-imx8mp-tac5212-masterV42.m4} (top-level)}

\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize]
#
# Topology V4.2 Phase 1a.1 MVP : console 8ch end-to-end, linked effects.
# 2 pipelines 8ch autonomes, SCHEDULE_TIME_DOMAIN_DMA partout (12e arg).
...
# 12e arg SCHEDULE_TIME_DOMAIN_DMA ajouté V4.2 (hygiène, cohérence DAI_ADD)
...
% =======================================================================
\section{Test T5 bit-perfect --- stratégie T5b recommandée}\label{sec:t5}

\textbf{Précision V4.2} : \texttt{drc.h:26} définit \texttt{CONFIG\_DRC\_MAX\_PRE\_DELAY\_FRAMES = 512} et \texttt{drc.h:29} définit \texttt{DRC\_DEFAULT\_PRE\_DELAY\_FRAMES = CONFIG\_DRC\_MAX\_PRE\_DELAY\_FRAMES >> 1 = 256} frames. Le pre-delay effectif par défaut = 256 frames ($\sim$5.3 ms @48k) mais le max configurable est 512.

Même avec \texttt{enabled=0} côté DRC, la fonction \texttt{drc\_s32\_default} (\texttt{drc\_generic.c:744-750}) appelle \texttt{drc\_delay\_input\_sample\_s32} qui applique le pre-delay. La fonction zero-latency \texttt{drc\_default\_pass = audio\_stream\_copy} (\texttt{drc\_generic.c:473}) n'est sélectionnée \textbf{que si \texttt{cd->config == NULL}} (\texttt{drc.c:362}). Dès qu'un blob est chargé, le pre-delay est inévitable.
...
\begin{enumerate}[leftmargin=*, itemsep=1pt]
  ...
  \item \textbf{T5b sans DRC \textcolor{cok}{(RECOMMANDÉ V4.2)}} : topology variante \texttt{sof-imx8mp-tac5212-masterV42-T5.m4} qui omet \texttt{W\_DRC} et le buffer associé. Graph réduit : \texttt{DAI RX $\to$ B2 $\to$ MBDRC $\to$ B1 $\to$ PGA $\to$ B0 $\to$ HOST}. Null test sample-perfect atteignable (MBDRC en passthrough blob + PGA bit-perfect à \texttt{VOL\_ZERO\_DB=0x10000}). \textbf{Seule voie sample-exact} confirmée par les 6 workers.
  \item \textbf{T5c blob patché} : créer localement un \texttt{drc\_coef\_zerolatency.m4} avec \texttt{enabled=0} ET \texttt{pre\_delay\_time=0}. Mais \texttt{drc\_set\_pre\_delay\_time()} clampe à \texttt{DRC\_DIVISION\_FRAMES=32} min $\to$ delay minimum 32 frames garanti. Non sample-exact. Abandonné.
\end{enumerate}

\textbf{Choix V4.2} : \textbf{T5b} pour validation bit-perfect stricte de la gate. La topology MVP principale (\texttt{masterV42.m4}) reste avec DRC actif pour les tests T1-T4 (trigger, full-duplex, contrôles).

% =======================================================================
\section{Tests Phase 1a.1 MVP V4.2}

\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{T0 Compile} : \texttt{m4 -I. -I m4 -I sof -I common -I platform/common common/abi.m4 masterV42.m4 | alsatplg -c - -o masterV42.tplg} sans erreur, aucun widget orphelin. \textbf{Gate obligatoire} avant deploy.
  \item \textbf{T0a Widget count} : \texttt{alsatplg --dump masterV42.tplg | grep -c "type: scheduler"} doit retourner \textbf{2} (preuve : 1 scheduler par pipeline, créé par pipe-dai-*.m4). Total $\sim$22 widgets.
  \item \textbf{T1 Load} : kernel charge sans \texttt{-22} ni \texttt{-110}. Carte \texttt{softac5212tdm} + 2 PCMs visibles.
  ...
  \item \textbf{T5 Bit-perfect} : stratégie \textbf{T5b} avec topology variante \texttt{masterV42-T5.m4} (sans \texttt{W\_DRC}). Loopback HW TAC OUT $\to$ TAC IN, null test via \texttt{sox -m -v 1 in.wav -v -1 rec.wav null} $\to$ attendu $\leq 1$ LSB.
  \item \textbf{T6 Contrôles} : \texttt{amixer controls | grep -E "Master|MBDRC|DRC"} liste "1 Master Capture Volume", "6 Master Playback Volume", 4 byte controls MBDRC/DRC. \texttt{amixer cset name='1 Master Capture Volume' 32} change le gain sans glitch.
  \item \textbf{T7 Charge CPU} : lecture via \texttt{sof-logger -t /etc/sof/sof-logger.toml -f /lib/firmware/imx/sof/sof-imx8m.ldc} ou debugfs \texttt{/sys/kernel/debug/sof/etrace} des cycles \texttt{kcps} ou \texttt{cpc}. Attendu $<$ 30\%.
\end{enumerate}
...
\textbf{Stratégie V4.2 retenue} : \textbf{option co-scheduled cross-pipeline same sched\_comp} (pattern \texttt{sof-imx8mp-tac5212-masterlite.m4:49-56} confirmé upstream avec \texttt{PIPELINE\_SCHED\_COMP\_3}).

\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item Pipeline playback (PIPE 6) enrichie : \texttt{W\_MUXDEMUX} inséré en fin de chaîne effets alimente 2 sinks : B3 (vers DAI TX) et B4 (vers pipeline capture HP\_Monitor). Export \texttt{PIPELINE\_DEMUX\_6 = N\_MUXDEMUX(0)}.
  \item Pipeline HP\_Monitor (PIPE 7) : pipeline capture avec 13e arg \texttt{PIPELINE\_PLAYBACK\_SCHED\_COMP\_6} (exporté par \texttt{pipe-dai-playback.m4:24}). Le \texttt{sched\_comp = N\_DAI\_OUT(6)} est partagé $\to$ guard \texttt{is\_same\_sched} (\texttt{pipeline-stream.c:505}) passe.
  \item DAPM cross-pipeline : \texttt{SectionGraph} top-level avec \texttt{dapm(PIPELINE\_SINK\_7, PIPELINE\_DEMUX\_6)}. Le widget \texttt{PIPELINE\_DEMUX\_6} doit être \textbf{explicitement exporté} par le fichier playback (bug V2.7 à ne pas reproduire : \texttt{masterlite.m4:88} référencait \texttt{PIPELINE\_DEMUX\_3} non-exporté).
\end{itemize}

Fallback : loopback HW (câble TAC OUT $\to$ TAC IN) si le pattern cross-pipeline tap reste instable.

% =======================================================================
\section{Livrables Phase 1a.1 MVP V4.2}

\begin{itemize}[leftmargin=*, itemsep=1pt]
  \item \texttt{sof/tools/topology/topology1/sof-imx8mp-tac5212-masterV42.m4} (top-level)
  ...
  \item \texttt{sof/tools/topology/topology1/sof-imx8mp-tac5212-masterV42-T5.m4} (variante T5 sans \texttt{W\_DRC})
  \item Patch \texttt{sof/app/boards/imx8mp\_evk\_mimx8ml8\_adsp.conf} : \texttt{+CONFIG\_COMP\_MIXER=y}
  \item Entrée \texttt{sof/tools/topology/topology1/CMakeLists.txt} \texttt{TPLGS} : ajouter \texttt{"sof-imx8mp-tac5212-masterV42;sof-imx8mp-tac5212-masterV42;-DSAI\_INDEX=7"} (ou compile via recipe Yocto meta-local directement)

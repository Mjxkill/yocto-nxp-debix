#!/usr/bin/env python3
"""
V9.5.3-v5.15 — Phase 3 — DEEP Conv2D + LR cosine + loss compression boostée.

Différences vs v5.14 :
- **Modèle DEEP_conv2d** (5M params, 8 couches Conv2D)
  → 10× plus de capacité ; bench NPU 1.66 ms (15× plus rapide que CPU)
- **LR cosine annealing** 3e-4 → 1e-6 sur 30 epochs (vs LR fixe 1e-4)
- **Loss compression BOOSTÉE** : weight ×5 sur Δ crest + Δ dynamic range
  → force le modèle à apprendre la compression dynamique
- Bandes EQ fixes héritées v5.14 (16 ISO + Q=1.0)

Cache réutilisé : pair_cache_v5_14_200ms_aug.npz (paires × augmentations).
"""
...
from model import (
    MasteringDEEP_conv2d, denormalize_params_v5_14,
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.15 — DEEP Conv2D + cosine LR + loss compression ×5")
    ...
    model = MasteringDEEP_conv2d(n_input=N_FEATURES_IN_V2).to(DEVICE)
    print(f"Model: MasteringDEEP_conv2d — {n_parameters(model):,} params (output={N_PARAMS_OUT_V5_14})")
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_15')
    args = ap.parse_args()

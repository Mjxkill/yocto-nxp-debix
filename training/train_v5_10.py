#!/usr/bin/env python3
"""
V9.5.3-v5.10 — Window 1s pour réduire les ondulations + delta-matching.
...
CHUNK_SEC = 1.0                    # V5.10 : window 1s pour stabilité (vs 200ms v5.9)
SR        = 48000
...
_log_edges  = np.logspace(np.log10(20.0), np.log10(1000.0), 7)
_lin_edges  = np.linspace(1000.0, 20000.0, 15)
_edges      = np.concatenate([_log_edges[:-1], _lin_edges])  # 21 edges total
...
def loss_v5_9(raw, output, target):
    """V5.9 — Delta-matching : compare ce que la chain a ajouté vs ce que
    le master humain a ajouté, par bande.

    delta_out = bands_dB(output) - bands_dB(raw)   # ce que LV2 chain fait
    delta_tgt = bands_dB(target) - bands_dB(raw)   # ce que le master HUMAIN fait
    """
    raw_b = rms_db_per_band_stereo(raw, SR)        # (B, 20)
    out_b = rms_db_per_band_stereo(output, SR)
    tgt_b = rms_db_per_band_stereo(target, SR)

    delta_out = out_b - raw_b                      # (B, 20)
    delta_tgt = tgt_b - raw_b

    # Clamp pour stabilité (évite divergence type v5.7)
    delta_out_c = torch.clamp(delta_out, -20.0, +20.0)
    delta_tgt_c = torch.clamp(delta_tgt, -20.0, +20.0)

    # Loss niveau exact par bande
    L_delta_band = ((delta_out_c - delta_tgt_c) ** 2).mean()
    # Loss forme du delta (Pearson sur les deltas)
    L_delta_corr = pearson_corr_loss(delta_out_c, delta_tgt_c)

    # RMS et crest globaux (orthogonaux)
    ...
    return {
        ...
        'corr_one_minus':  L_delta_corr.detach(),         # = delta corr
        'band_delta_db':   L_delta_band.detach().sqrt(),  # = écart abs des deltas
    }
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.10 — window 1s + delta-matching + exciter freq min 5kHz")
    print(f"  chunk_sec={CHUNK_SEC} lr={lr} batch={batch_size} epochs={epochs}")

    # V5.5 : cache dédié 200 ms
    # V5.10 : utilise le cache 1s (mêmes features mid_only)
    ...
    for epoch in range(epochs):
        ...
        for b in range(n_batches):
            ...
            losses = loss_v5_9(raws, output, targets)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_10')
    args = ap.parse_args()

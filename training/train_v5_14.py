#!/usr/bin/env python3
"""
V9.5.3-v5.14 — Phase 3 — full dataset + bigger model + augmentation + loss comp/perceptuelle.

Différences vs v5.13 :
- **Bandes EQ FIXES** (16 ISO + Q=1.0). Le modèle prédit UNIQUEMENT les
  16 gains_db + 14 plugin params = **30 outputs** (vs 62).
- Aligne avec pratique mastering pro (SSL/API/multi-band fixed bands).
- Modèle plus simple → meilleure généralisation, moins de params à push en
  runtime (16 gains EQ au lieu de 48 EQ × cycle = ×3 moins de set_param).

Améliorations héritées v5.13 :
1. Full dataset 259 paires (symlinks réparés)
2. Modèle MasteringConv1DLargeV5_14 ~500K params (vs 52K v5.12)
3. Data augmentation : random gain ±6 dB, channel swap, pre-EQ
4. Loss compression : pénalise Δ crest + Δ dynamic range par bande
5. Loss perceptuelle : L1 sur log Mel-spectrogram

Cache dédié : pair_cache_v5_14_200ms_aug.npz (paires × augmentations).
"""
...
from model import (
    MasteringConv1DLargeV5_14, denormalize_params_v5_14,
    N_FEATURES_IN_V2, N_PARAMS_OUT_V5_14, n_parameters,
)
...
def _build_pair_cache_200ms(max_pairs):
    ...
    cache_path = CACHE_DIR / 'pair_cache_v5_14_200ms_aug.npz'
    ...
    for i, pair in enumerate(iter_pairs()):
        ...
        if n_chunks == 0:
            continue
        # V5.14 — subsample : prend N_CHUNKS_PER_PAIR_MAX chunks aléatoires
        # par paire pour éviter explosion de la durée d'epoch.
        if n_chunks > N_CHUNKS_PER_PAIR_MAX:
            chunk_indices = rng.choice(n_chunks, size=N_CHUNKS_PER_PAIR_MAX, replace=False)
        else:
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.14 — bandes FIXES + bigger model + augmentation + loss comp/perceptuelle")
    ...
    model = MasteringConv1DLargeV5_14(n_input=N_FEATURES_IN_V2).to(DEVICE)
    print(f"Model: MasteringConv1DLargeV5_14 — {n_parameters(model):,} params (output={N_PARAMS_OUT_V5_14})")
    ...
    for epoch in range(epochs):
        ...
        for b in range(n_batches):
            ...
            params = denormalize_params_v5_14(params_norm)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_14')
    args = ap.parse_args()

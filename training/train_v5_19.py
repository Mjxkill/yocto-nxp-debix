#!/usr/bin/env python3
"""
V9.5.3-v5.19 — enveloppe spectrale fine 64 pts (remplace l'EQ 16 bandes).
...
from surrogate_spectral_env import SpectralEnvSurrogate
...
SCALE_SMOOTH   = 4.0     # V5.19 : douceur enveloppe ; marche 2 dB entre bandes → 4/4 = 1
...
def loss_v5_12(raw, output, target, params_norm=None, env_db=None):
    ...
    total = (L_rms_n + L_delta_band_n + L_delta_corr_n + L_crest_n
             + L_compress_n + L_mel_n + L_tilt_n + L_smooth_n)
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.19 — enveloppe spectrale 64 pts + exciter + limiter (EQ 16 bandes retiré)")
    ...
    for epoch in range(epochs):
        ...
        ep_L_comp = ep_L_mel = ep_L_mc = ep_L_tilt = ep_L_smooth = 0.0
        ep_rms  = 0.0
        ep_corr = 0.0
        ...
        print(f"  epoch {epoch:>3d} loss={m_loss:.2f} | "
              ...
              f"L_comp={rec['L_compress']:.2f} L_mel={rec['L_mel']:.2f} L_tilt={rec['L_tilt']:.2f} L_sm={rec['L_smooth']:.2f} "
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_19')
    args = ap.parse_args()

#!/usr/bin/env python3
"""
V9.5.3-v5.2 — Pondérations équilibrées (option A) suite divergence v5.1.

v5.1 (RMS=15, corr=3) → epoch 1 corr=0.43 RMS=2.98 puis divergence epoch 3.
Diagnostic : L_rms dominant + L_corr poussant fort = oscillation.

Option A : réduire L_rms (15→8) et L_corr (3→2) pour équilibrer.

Loss :
    1.0 * MSE_spectral_global   (inchangé)
 +  8.0 * ΔRMS_dB²              ← réduit 15 → 8
 +  0.5 * Δcrest²               ← inchangé
 +  2.0 * MSE_band_db           ← inchangé
 +  2.0 * (1 - corr_Pearson)    ← réduit 3 → 2
"""
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.2 — option A : L_rms 15→8, L_corr 3→2")
    ...
    for epoch in range(epochs):
        ...
        for b in range(n_batches):
            ...
            losses = loss_v5_2(output, targets)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_2')
    args = ap.parse_args()

#!/usr/bin/env python3
"""
V9.5.3-v5.7 — 20 bandes log-spaced + MSE par bande dB (au lieu de Pearson).

v5.5 utilisait 5 bandes : la corrélation Pearson sur 5 points est trop grossière.
v5.6 : 20 bandes log-spaced (~1/4 octave) de 20 Hz à 20 kHz pour une enveloppe
spectrale beaucoup plus précise.

Cache identique à v5.5 (mêmes chunks 200 ms).

Loss inchangée :
   (ΔRMS_dB² / 25)
 + (1 - corr_Pearson_20bands)
 + (Δcrest² / 100)
"""
...
# V5.6 : 20 bandes log-spaced de 20 Hz à 20 kHz (~1/4 octave par bande).
_edges = np.logspace(np.log10(20.0), np.log10(20000.0), 21)
...
SCALE_BAND = 25.0    # ΔbandRMS² typique (~ 5dB par bande au début)


def loss_v5_4(output, target):
    """V5.7 — 3 losses normalisées : RMS global + MSE per-band (20 bandes) + crest."""
    ...
    # V5.7 : MSE par bande au lieu de Pearson
    L_band = ((out_b - tgt_b) ** 2).mean()

    L_rms_n   = L_rms   / SCALE_RMS
    L_band_n  = L_band  / SCALE_BAND
    L_crest_n = L_crest / SCALE_CREST

    total = L_rms_n + L_band_n + L_crest_n
    ...
    return {
        ...
        # gardé pour compat colonne 'corr_one_minus' du parsing — ici on met
        # band_delta normalisé (plus bas = mieux)
        'corr_one_minus': L_band.detach().sqrt(),
        'contrib_rms':    L_rms_n.detach(),
        'contrib_band':   L_band_n.detach(),
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.7 — 20 bandes + MSE per-band (RMS abs par bande)")
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_7')
    args = ap.parse_args()

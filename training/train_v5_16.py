#!/usr/bin/env python3
"""
V9.5.3-v5.16 — Phase 3 — Mel+MFCC encoder + ResNet Conv2D + anti-mode-collapse.

Différences vs v5.15 :
- **Encoder Mel+MFCC** : 85 features par trame (vs 11) — features.py → features_v2.py
- **Chunks 100ms hop 100ms** : 10 trames par chunk (vs 200ms / 19 trames v5.15)
- **Modèle MasteringResNet_conv2d** (1.7M params, skip connections + Kaiming init)
  → résout le mode collapse de v5.15 (modèle prédisait constante)
- **Anti-mode-collapse loss** : pénalise std=0 sur batch (force diversité)
- Bandes EQ fixes héritées v5.14 (16 ISO + Q=1.0)
- Loss compression boostée héritée v5.15

Cache : pair_cache_v5_16_mel_100ms.npz (paires × aug × chunks 100ms).
"""
...
from features_v2 import compute_features_v2, N_FEATURES_V2
from model import (
    MasteringXXL_conv2d, denormalize_params_v5_14,
    N_FEATURES_IN_V3, N_PARAMS_OUT_V5_14, n_parameters,
)
...
EPOCHS_DEFAULT = 30
LR_DEFAULT     = 3e-4    # V5.15 : LR plus haut + cosine (vs 1e-4 + ReduceOnPlateau)
...
def _build_pair_cache_200ms(max_pairs):
    ...
    cache_path = CACHE_DIR / 'pair_cache_v5_16_mel_100ms.npz'
    ...
    for i, pair in enumerate(iter_pairs()):
        ...
        for c in chunk_indices:
            ...
            for aug_idx in range(N_AUGMENTATIONS):
                ...
                feats = compute_features_v2(r, sr=SR)
                if len(feats) == 0:
                    continue
                pairs.append({
                    'raw': r.T,
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.16 — Mel+MFCC encoder + ResNet Conv2D + anti-mode-collapse")
    ...
    model = MasteringXXL_conv2d(n_input=N_FEATURES_IN_V3).to(DEVICE)
    print(f"Model: MasteringXXL_conv2d — {n_parameters(model):,} params (in={N_FEATURES_IN_V3}, out={N_PARAMS_OUT_V5_14})")
    ...
    for epoch in range(epochs):
        ...
        ep_L_comp = ep_L_mel = ep_L_mc = 0.0
        ep_rms  = 0.0
        ep_corr = 0.0
        ...
        print(f"  epoch {epoch:>3d} loss={m_loss:.2f} | "
              ...
              f"L_comp={rec['L_compress']:.2f} L_mel={rec['L_mel']:.2f} "
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_16')
    args = ap.parse_args()

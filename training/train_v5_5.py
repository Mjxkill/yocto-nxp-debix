#!/usr/bin/env python3
"""
V9.5.3-v5.5 — Window 200 ms pour compression rapide type compresseur HW.

v5.4 utilisait des chunks 1s qui créaient des oscillations audibles quand un
nouvel instrument arrive (trompette etc.). Le mastering "compresseur" doit
réagir en <100 ms aux transients.

v5.5 :
  - CHUNK_SEC = 0.2 s  (9600 samples @ 48 kHz)
  - Cache dédié : pair_cache_v5_5_200ms.npz
  - Features ~19 frames par chunk
  - Loss identique à v5.4 (normalisée)
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.5 — window 200 ms + losses normalisées")
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_5')
    args = ap.parse_args()

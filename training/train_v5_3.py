#!/usr/bin/env python3
"""
V9.5.3-v5.3 — Losses ORTHOGONALES (objectifs non-redondants).

v5.1 et v5.2 ont 5 losses qui mesurent le spectre sous 5 angles → elles se
chevauchent → gradients se contredisent → divergence ou trade-offs sales.

v5.3 : 3 losses qui mesurent 3 aspects ORTHOGONAUX (pas de redondance).

Loss :
    8.0 * ΔRMS_dB²              ← LOUDNESS global (indépendant de la forme)
 +  4.0 * (1 - corr_Pearson)    ← SHAPE spectrale (indépendant du niveau)
 +  0.5 * Δcrest²               ← DYNAMIQUE (peak/RMS ratio)

Retiré : L_spectral (redondant avec corr+rms), L_band (redondant avec rms+corr)
"""
...
def loss_v5_3(output, target):
    """V5.3 — 3 losses orthogonales :
       - L_rms : loudness global, indépendant de la forme
       - L_corr : forme spectrale, indépendant du niveau
       - L_crest : dynamique, indépendant des 2 autres
    """
...
def train(n_pairs=30, epochs=EPOCHS_DEFAULT, lr=LR_DEFAULT,
    ...
    print(f"V9.5.3-v5.3 — losses orthogonales (rms / corr / crest)")
    ...
    for epoch in range(epochs):
        ...
        for b in range(n_batches):
            ...
            losses = loss_v5_3(output, targets)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--tag', type=str, default='v5_3')
    args = ap.parse_args()

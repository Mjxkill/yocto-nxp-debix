...
Sorties : eval_v5_20_ep<N>/<slug>_{raw,model,target}.wav
"""
...
CKPT      = WORKSPACE / 'checkpoints' / f'conv_v5_20_full_epoch{EPOCH:03d}.pt'
OUT_DIR   = WORKSPACE / f'eval_v5_20_ep{EPOCH}'
...
def main():
    ...
    for pair in pairs:
        ...
        # V5.20 — cap anti-souffle identique au training : ±6 dB > 8 kHz
        from surrogate_spectral_env import ENV_FREQS
        hf = ENV_FREQS > 8000.0
        envs[:, hf] = np.clip(envs[:, hf], -6.0, 6.0)

...
| **B0/B2 ✗** | **IRQ affinity DWC3 testé empiriquement 2026-05-17** : mask 0xc (cores 2,3) EMPIRE (wr/rd max 2700→5054 / 2946→7419 µs, cap_full 0→15745 events/30s) — cache contention massive avec audio_thread. Mask 0xf (default core 0) reste optimal. Théorie 28M IRQs core 0 = bottleneck rejetée par mesure. | **REJETÉ empiriquement** |
| **V8.34 PLC ✗** | **Tentative PLC cap ring** : répétition last valid period au lieu memset zéros sur cap_empty, fade Q8 ramp. Commit `fb86e3d8` puis revert `18570966`. | **REVERT 2026-05-17** — utilisateur entendait glitchs ET "musique de robot" sur certains blocs alors que `plc_events=2`. Diagnostic : la PLC fonctionnait correctement (corrélation `cap_empty==plc_events`), mais les glitchs viennent d'ailleurs (cap_full 728/s = USB push drop, ou ASRC drop artefact). Test `--no-asrc` empire (drift -220 → -13 swing). Test `--fixed-shift 13` cause robot music sur USB stream death. Path PLC abandonné, retour V8.33. |
| **V8.x suite — ASRC track plus fin** | **Hypothèse à explorer** : ASRC actuel compense partiellement (drift +11 stable au lieu de 0). Les artefacts audibles scalent avec \|drift\|. Pistes : compute_correction plus réactif (interval plus court), drop pattern uniforme (smooth interp vs fusion 2-en-1 brutale), ou fill-based correction (les UAC2_FILL_HIGH constantes définies mais non utilisées). | **À INVESTIGUER — vrai bottleneck identifié** |
| **V9.0 RT + pinning ✓** | **PREEMPT_RT kernel + per-thread CPU pinning + RT priorities** : patch `patch-6.6.36-rt35` de kernel.org appliqué via SRC_URI bbappend. `CONFIG_PREEMPT_RT=y` forcé dans do_configure (merge_config silently rejette sinon). Bootargs += `threadirqs`. mixer-pro V9.0 : `pthread_setaffinity_np` par thread (audio+play → core 2, uac2 → core 3, analyzer+control → cores 0,1). Prios : audio 99, play 98, uac2_cap/play 95, analyzer 60. Service `CPUAffinity=0 1 2 3` (élargi pour autoriser pinning explicite). | **DÉPLOYÉ 2026-05-24** — cyclictest max **1.2 ms → 53 µs (20× gain)**. prof_iter_us 2553 → 1999 µs (target tenu). Mais : pics > 3 ms iter restaient à 17/90s (cap_dsp readi prend 2.2-2.9 ms parfois au lieu de 1.4 ms). |
| **V9.1 IRQ kthread prio ✓** | **Daemon `irq-prio-rt`** : sous PREEMPT_RT les IRQ kthreads default prio 50, préemptés par audio_thread prio 80 → priority inversion → readi cap_dsp attend la notif mailbox. Daemon poll 5 s, bump 6 IRQ critiques à prio 90 : `irq/229-30e60000.mailbox[3-0/1]` (IPC A53↔HiFi4), `irq/15-30bd0000.dma-controller` (SDMA1), `irq/214-30e10000.dma-controller` (SDMA2 audio), `irq/23-32f10100.usb`, `irq/24-32f10108.usb` (DWC3 x2). Mode daemon nécessaire car mailbox kthread est respawné quand SOF se ré-init. Nouveau recipe `meta-local/recipes-bsp/irq-prio-rt/`. `mixer-pro.service` auto-enable. | **GO 2026-05-24** — Test Tauri MP3 90 s : **17 pics > 3 ms → 3 pics (-82%)**. cap_full 728/s → 0-48/s. Utilisateur valide *"c'est beaucoup mieux"*. Glitchs résiduels rares (origine au-delà du tuning Linux RT : DSP firmware interne ou NoC contention DDR). Commit `619b21e8`. Fiche `TESTS_V9.1.md`. |
| **V9.x suite — diminishing returns** | **Pics résiduels (3/90s) hors scope Linux RT.** Pistes futures à effort/risque décroissant : (1) NoC QoS audio/USB priority, (2) ALSA buffer cap_dsp 4→8 periods (+8 ms latence), (3) backport `p_sync=async` f_uac2 kernel 6.10→6.6, (4) SOF firmware tuning (critic_analyze obligatoire). | **À évaluer si glitch utilisateur résiduel inacceptable** |

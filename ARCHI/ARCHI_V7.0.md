...
| **E7.2** | **UI premium "Apple-class"** : layout groupé (DSP / UAC2 ASIO / Phone / Bus FX / Master), faders verticaux DAW-style, dark glassmorphism, animations fluides, typo SF Pro / Inter | UI visuellement comparable Logic Pro / Ableton |
| **E7.3** | **Panels effets DSP complets** : pour chaque bus FX (compressor/reverb/delay/EQ), tous params exposés en knobs rotatifs visuels (threshold, ratio, attack, release, makeup, size, damp, wet, time, feedback, freq L/M/H, gain L/M/H, Q) | Tous params modifiables en live |
| **E7.4** | **Effets TAC5212** : ADC chain (AGC, HPF, biquads, decim) + DAC chain (DRC, biquads, foldback, limiter, VAD, tone gen). Backend `tac-kcontrols` lit/écrit via libasound kcontrols | Réglages TAC pilotables, persisté ALSA state |
| **E7.5** | **Spectre + phase FFT** : par voie sélectionnée, calcul FFT côté backend (kissfft), stream SSE binary, canvas spectrogramme + scope phase X-Y | Spectre 1024 bins @ 30 Hz |

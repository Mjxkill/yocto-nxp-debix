...
| **E6.e ✓ MVP** | 4 effets natifs C sur les bus FX : compressor, reverb Schroeder, delay, EQ 3-band biquad. `set_fx_param` via socket JSON | **GO MVP 2026-05-11** — vtable `fx_engine_t` (prêt LV2 futur), 8 params modifiés en live OK, routage produit signal (-92 dB vs -inf sans). ⚠️ Throughput effets actifs 30-50 % nominal (sample-par-sample non vectorisé → E6.f block+NEON), validation perceptuelle non mesurée |
